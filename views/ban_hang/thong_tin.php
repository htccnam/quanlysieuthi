...
// Tính doanh thu trung bình (tránh chia cho 0)
$average_per_order = ($total_orders > 0) ? ($total_revenue / $total_orders) : 0;

// 3. Truy vấn danh sách đơn hàng để hiển thị trong bảng
...
<head>
    ...
    <style>
        /* GIỮ NGUYÊN GIAO DIỆN CSS CỦA BẠN */
            ...
            <tbody>
                <?php if ($res_list->num_rows > 0): ?>
                    ...
                    <tr>
                        ...
                        <td><?= $row['thanhtoan'] ?></td>
                        <td><?= $row['makhuyenmai'] ? $row['makhuyenmai'] : 'Không' ?></td>
                    ...
                    <?php endwhile; ?>
                <?php else: ?>
                    <tr><td colspan="7" style="text-align:center;">Chưa có dữ liệu đơn hàng nào.</td></tr>
                <?php endif; ?>
            </tbody>
        </table>
    </div>
